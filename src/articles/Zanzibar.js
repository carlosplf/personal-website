    ...
    getBody() {
        return(
            <div className="ArticleBody">
                ...
                <p className="NormalText">Os modo como as ACLs são armazenadas impactam todo o funcionamnto do sistema. A proposta do Zanzibar é responder rapidamente para ecritas e leituras nas permissões. Ao editar um Documento no Google Docs, por exemplo, o usuário espera que as alterações sejam salvas de modo instantâneo e imperceptível. O usuário também espera que as verificações de  permissões sejam consistentes e imediatas.</p>
                
                <p className="NormalText"> Os dados de relações (ACLs) são salvos em Banco de Dados (BDs) separados para cada <span className="italic">Namespace</span>. Cada cliente que utiliza o Zanzibar define o seu <span className="italic">Namespace</span>, com base no ID do Documento e os nomes das relações. Cada relação, inclusive em cada versão, é salva em uma tupla diferente no BD. Isso facilita para que o Zanzibar consiga enxergar as versões diferentes de cada ACL, bem como o timestamp de cada uma. Na grande maioria dos casos, o ID único de cada relação (Shard ID) no banco é definido pelo próprio ID do Objeto. Em alguns casos, o sistema pode configurar o Shard ID com base no ID do Objeto e nos IDs de Usuários. O Sistema também mantém um <span className="italic">changelog</span>  das alterações de cada ACL em suas versões. Salvar o <span className="italic">changelog</span> é importante para que o sistema consiga responder rapidamente requisições de consulta de ACLs e suas alterações.</p> 

                <p className="NormalText">Os <span className="italic">Namespaces</span> são armazenados em um BD com duas tabelas. Uma para salvar o <span className="italic">Namespace</span> e suas configurações, e outra para armazenar o <span className="italic">changelog</span> dos <span className="italic">Namespaces</span>.</p>

                <p className="NormalText">Com o objetivo de diminuir latência para os clientes, o Zanzibar replica os dados de ACLs e Namespaces para regiões mais próximas do cliente. São dezenas de réplicas para cada região do globo.</p>

                <p className="NormalText">Para conseguir responder requisições de <span className="italic">Check</span> e <span className="italic">Expand</span> rapidamente, o Zanzibar se apoia firmemente em um sistema de indexação chamado <span className="bold italic">Leopard</span>. O Leopard mantém um registro em meória de subgrupos de hierarquia, que pertencem a outros grupos de mais alto nível. Dessa forma, o Leopard consegue responder mais rapidamente às consultas de grupos e subgrupos aninhados. O Leopard funciona como um sistema independente ao core do Zanzibar, e é consultado pela API de Watch, descrita previamente neste artigo.</p>
                
                <p className="SubTitle">Conclusão</p>
                
                <p className="NormalText">Lidar com permissões em ambientes computacionais está longe de ser uma tarefa fácil, ainda mais considerando o cenário dos produtos Google, que compartilham uma infraestrutura de permissões e controle de acesso. É por esse motivo que o Zanzibar é peça fundamental para que serviços essenciais como Google Drive, Photos e Maps possam ser oferecidos aos usuários de forma eficiente.</p> 
